...
tmpdir=$(mktemp -d "${TMPDIR:-/tmp}/wavevm-runtime-profile-publication.XXXXXX")
trap 'rm -rf "$tmpdir"' EXIT

gcc -Wall -Wextra -Werror -std=c11 -D_POSIX_C_SOURCE=200809L \
    -ffunction-sections -fdata-sections \
    -I"$repo_root/common_include" -I"$repo_root/ctl_tool" \
    "$repo_root/ctl_tool/runtime_profile_publication.c" \
    ...
    "$repo_root/tests/control-plane/test_runtime_profile_publication.c" \
    -Wl,--gc-sections -pthread -o "$tmpdir/test_runtime_profile_publication"
"$tmpdir/test_runtime_profile_publication"
