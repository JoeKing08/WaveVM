...
tmpdir=$(mktemp -d "${TMPDIR:-/tmp}/wavevm-admission-provider-dynamic.XXXXXX")
...
gcc -Wall -Wextra -Werror -std=c11 -D_POSIX_C_SOURCE=200809L \
    ...
    "$repo_root/tests/control-plane/test_admission_provider_dynamic.c" \
    -o "$tmpdir/test_admission_provider_dynamic"
"$tmpdir/test_admission_provider_dynamic"
