...
tmpdir=$(mktemp -d "${TMPDIR:-/tmp}/wavevm-admission-slot-registry.XXXXXX")
...
gcc -Wall -Wextra -Werror -std=c11 -D_POSIX_C_SOURCE=200809L \
    ...
    "$repo_root/common_include/wavevm_admission_slot_registry.c" \
    "$repo_root/tests/control-plane/test_admission_slot_registry.c" \
    -pthread -o "$tmpdir/test_admission_slot_registry"
"$tmpdir/test_admission_slot_registry"
